...
| `sms_push_token` | Token provider unifié : clé API pour `smsmobileapi`, Bearer pour `simple_sms_gateway`, X-Api-Key injecté dans les scripts Termux/MacroDroid. Si vide, les scripts Android utilisent `api_key`. |
...
3. Télécharge le script Python SMS depuis `GET /api/cabinet/termux/sms-queue` (pré-configuré par Grav)
...
```
GET /api/cabinet/sms/queue          → récupère les SMS status=prepared
  pour chaque SMS :
    termux-sms-send -n {to} {message}
    → succès : POST /api/cabinet/sms/queue/{id}/ack  {"status":"sent"}
    → échec  : POST /api/cabinet/sms/queue/{id}/ack  {"status":"error","error":"raison"}
```
...
|---------|-------|-------------|
| `GET` | `/api/cabinet/termux/bootstrap` | Script bootstrap pré-configuré (bash) — param optionnel `?sim=N` |
| `GET` | `/api/cabinet/termux/sms-queue` | Script SMS pré-configuré (Python) — param optionnel `?sim=N` |
...
| `GET` | `/api/cabinet/sms/queue` | Lister les SMS en attente (`status=prepared`, `channel=sms`) — utilisé par MacroDroid et Termux |
...
└── templates/
    ...
    ├── termux-bootstrap.sh.twig      # Bootstrap Termux pré-configuré (servi par /api/cabinet/termux/bootstrap)
    ├── termux-sms-queue.py.twig      # Script Python SMS pré-configuré (servi par /api/cabinet/termux/sms-queue)
    └── partials/cabinet/
