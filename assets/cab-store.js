/* Cabinet — Alpine store + data loading */
...
document.addEventListener('alpine:init', () => {
  Alpine.store('cab', {
    ...
    // Reactive list (rebuilt by renderList())
    clientList: [],
    searchQuery: '',
    loadState: 'idle',
    loadError: '',

    // Bilan tab async state
    ...
    get activeSessions() {
      return (this.sessions[this.activeId] || []).slice().reverse();
    },
    get activeSessionCount() {
      return (this.sessions[this.activeId] || []).length;
    },
    get preparationSms() {
      if(!this.activeId) return '';
      return buildPreparationSms(
        this.activeClient,
        this.sessions[this.activeId] || [],
        this.activeId
      );
    },
    ...
    selectClient(id) {
      ...
      delete _bilanFileCache[id];
      this.renderList();
      document.getElementById('app')?.classList.toggle('client-open', !!id);
      window.dispatchEvent(new CustomEvent('cabinet:client-selected', {detail: id}));
    },
    ...
    async load() {
      ...
      try{
        ...
        this.loadState = 'error';
        this.loadError = 'Impossible de charger les donnees';
        console.error('Erreur chargement données', e);
      }
    },
  });

  // Load data immediately once the store is registered.
